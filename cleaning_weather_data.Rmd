...
Make sure the file path on the code is updated to the correct folder. In this case we are working witht the weather data, so data is seved inside the weather folder. Update folder name if you are downloading other type of data.
...
w_complete <- bind_rows(w_10_14, w_15_16) %>%
  ...
  select(yr, timestamp, everything())
...
write_csv(w_complete, paste0(getwd(),"/clean_data/2010_2016_palmyera_weather_table.csv"))

## Finalized data forlder url
finalized_url <- "https://drive.google.com/drive/u/1/folders/1GUrTanuN0VfBLjJBzqp3HeE8a-JX5VK6"

drive_upload(media = paste0(getwd(),"/clean_data/2010_2016_palmyera_weather_table.csv"),
             path = as_id(finalized_url),
             name = "2010_2016_palmyra_weather_table.csv",
             type = "csv")

##Uploaded susccesfully: need to ceck NaN. When saving csv they tun into NAs. De we need to keep them as NaN?
